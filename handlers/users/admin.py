...
async def broadcast_text_post(call: CallbackQuery):
        ...
        if len(file_list2) >= p:
            try:
                ...
                await call.message.edit_text(
                            ...
                            f"<b>На пользователя 🗣 {file_list2[t][:-1]} ✅</b>\n\n"
    ...
    await call.message.answer("✅ <b>Рассылка Завершена</b> ✅", reply_markup=back_to_main_menu)    
        #
        #while i <= uzik:
        #    client = TelegramClient(StringSession(akaunt), api_id_x, api_hash_x)
        #    await client.connect()
        #    #pismo = print(f"Есть подключение к акаунту {akaunt}")
        #    await client.send_message("satanasat", "da")
        #    time.sleep(10)
...
@dp.callback_query_handler(text="go_start44")
async def broadcast_text_post(call: CallbackQuery):
    path = f'pics/broadcast/cicada.jpg'
    try:
        with open(path, 'rb') as f:
        ...
        try:
            ...
            if mm <= 40:
                ...
                try:
                    ...
                    ss = open('ussers.txt', 'r').readlines()
                    user = ss[a][:-1]
                    print(f"Беру юзик {user}")
                    time.sleep(2)
                    #me = await client.get_me()
                    akk = acaunt.split(".")[0]
                    print(f"С акаунта {akk} делаю рассылку")
                    time.sleep(2)
                    tyty = "AgACAgQAAxkBAAIDk2IVgXUOASy_B4yI3Z0GiPE1a5WhAALaujEbH9SoUPlxx780PB6VAQADAgADeQADIwQ"
                    await client.send_file(user, file=tyty, caption=sms)
                    result = await client(functions.users.GetFullUserRequest(id="me"))
                    nam = result.user.first_name
                    lnam = result.user.last_name
                    await call.message.edit_text(
